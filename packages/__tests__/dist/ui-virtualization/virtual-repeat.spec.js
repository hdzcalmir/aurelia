...
describe('ui-virtualization/virtual-repeat.spec.ts', function () {
    ...
    it('renders', function () {
        createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items" style="height: 50px">${item}</div>'), class App {
            constructor() {
    ...
    it('understands scroller container border', function () {
        createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items" style="height: 50px">${item}</div>', { border: 30 }), class App {
            constructor() {
                this.items = createItems();
            }
        }, virtualRepeatDeps);
        assert.deepStrictEqual(virtualRepeats[0].getDistances(), [
            0,
    ...
    it('understands scroller container padding', function () {
        createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items" style="height: 50px">${item}</div>', { padding: 30 }), class App {
            constructor() {
                this.items = createItems();
            }
        }, virtualRepeatDeps);
    ...
    it('understands non borderbox box-sizing', function () {
        createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items" style="height: 50px">${item}</div>', { padding: 30, boxSizing: 'content-box' }), class App {
            constructor() {
                this.items = createItems();
            }
        }, virtualRepeatDeps);
    ...
    it('rerenders when scrolled', function () {
        const { scrollBy, flush } = createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items" style="height: 50px">${item.name}</div>'), class App {
            constructor() {
                this.items = createItems();
            }
        }, virtualRepeatDeps);
    ...
    describe('mutation', function () {
        it('rerenders when removed at the start', function () {
            const { component, flush } = createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items" style="height: 50px">${item.name}</div>'), class App {
                constructor() {
                    this.items = createItems();
                }
            }, virtualRepeatDeps);
    ...
    function createScrollerTemplate(content, styles) {
        let { height = 600, padding = 0, border = 0, 
...
//# sourceMappingURL=virtual-repeat.spec.js.map