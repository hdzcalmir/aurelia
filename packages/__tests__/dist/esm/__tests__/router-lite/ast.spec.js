    ...
    for (const [rawComponent, component] of componentSpecs) {
        for (const [rawViewport, viewport] of viewportSpecs) {
            for (const scoped of [true, false]) {
                ...
                specs[`/${raw}`] = [new RouteExpression(true, new SegmentExpression(component, viewport, scoped), emptyQuerystring, null), url];
                specs[`/(${raw})`] = [new RouteExpression(true, new SegmentGroupExpression(new SegmentExpression(component, viewport, scoped)), emptyQuerystring, null), `(${url})`];
                specs[`/${raw}/${raw}`] = [new RouteExpression(true, new ScopedSegmentExpression(new SegmentExpression(component, viewport, scoped), new SegmentExpression(component, viewport, scoped)), emptyQuerystring, null), `${url}/${url}`];
                ...
                specs[`/${raw}?foo=bar`] = [new RouteExpression(true, new SegmentExpression(component, viewport, scoped), Object.freeze(new URLSearchParams('foo=bar')), null), `${url}?foo=bar`];
                specs[`/${raw}?foo=bar&fiz=baz`] = [new RouteExpression(true, new SegmentExpression(component, viewport, scoped), Object.freeze(new URLSearchParams('foo=bar&fiz=baz')), null), `${url}?foo=bar&fiz=baz`];
                specs[`/${raw}?foo=bar1&fiz=baz&foo=bar2`] = [new RouteExpression(true, new SegmentExpression(component, viewport, scoped), Object.freeze(new URLSearchParams('foo=bar1&fiz=baz&foo=bar2')), null), `${url}?foo=bar1&fiz=baz&foo=bar2`];
            }
        }
    }
    const comp = {
