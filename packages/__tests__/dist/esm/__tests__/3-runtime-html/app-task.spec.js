...
describe('3-runtime-html/app-task.spec.ts', function () {
    it('runs app task with right timing', async function () {
        const logs = [];
        const addAppTaskLog = (name) => logs.push(`task.${name}`);
        const addCompLog = (name) => logs.push(`comp.${name}`);
        const { tearDown } = await createFixture('${message}', class {
...
//# sourceMappingURL=app-task.spec.js.map