...
describe('ui-virtualization/virtual-repeat.spec.ts', function () {
    ...
    it('renders', async function () {
        await createFixture('<div style="height: 600px; overflow: auto;"><div virtual-repeat.for="item of items" style="height: 50px">${item}</div></div>', class App {
            constructor() {
                this.items = Array.from({ length: 100 }, (_, idx) => {
                    return { idx, name: `item${idx}` };
                });
            }
        }, virtualRepeatDeps).started;
        assert.deepStrictEqual(virtualRepeats[0].getDistances(), [0, (100 - (600 / 50) * 2) * 50]);
    });
    it('rerenders when scrolled', async function () {
        const { scrollBy, ctx } = await createFixture('<div id=scroller style="height: 600px; overflow: auto;"><div virtual-repeat.for="item of items" style="height: 50px">${item.name}</div></div>', class App {
            constructor() {
                this.items = Array.from({ length: 100 }, (_, idx) => {
                    return { idx, name: `item${idx}` };
                });
