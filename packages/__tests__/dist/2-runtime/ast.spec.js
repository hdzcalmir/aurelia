...
function throwsOn(method, msg, ...args) {
    let err = null;
    try {
...
describe('2-runtime/ast.spec.ts', function () {
    ...
    describe('AccessKeyedExpression', function () {
        ...
        describe('returns the right value when accessing keyed on primitive', function () {
            it('returns string when accessing string character', function () {
                const value = astEvaluate(new AccessKeyedExpression(new PrimitiveLiteralExpression('a'), new PrimitiveLiteralExpression(0)), null, null, null);
                assert.strictEqual(value, 'a');
            });
            it('returns undefined when accessing keyed on null/undefined', function () {
                const value = astEvaluate(new AccessKeyedExpression(PrimitiveLiteralExpression.$null, new PrimitiveLiteralExpression(0)), null, null, null);
                assert.strictEqual(value, undefined);
            });
            it('returns prototype method when accessing keyed on primitive', function () {
                const value = astEvaluate(new AccessKeyedExpression(new PrimitiveLiteralExpression(0), new PrimitiveLiteralExpression('toFixed')), null, null, null);
    ...
    describe('AccessScopeExpression', function () {
        ...
        it(`connects undefined property on first ancestor bindingContext`, function () {
            const scope = createScopeForTest({ abc: 'xyz' }, {});
            scope.parent.parent = Scope.create({}, { foo: 'bar' });
            const binding = new MockBinding();
            astEvaluate($parentfoo, scope, dummyLocator, binding);
            assert.strictEqual(binding.calls.length, 1, 'binding.calls.length');
            assert.deepStrictEqual(binding.calls[0], ['observe', scope.parent.bindingContext, 'foo'], 'binding.calls[0]');
    ...
    describe('BinaryExpression', function () {
        ...
        it(`adds numbers`, function () {
            ...
            scope = createScopeForTest({});
            assert.strictEqual(isNaN(astEvaluate(expression, scope, null, null)), false, `isNaN(astEvaluate(expression, scope, null, null)`);
    ...
    describe('DestructuringAssignmentExpression', function () {
        ...
        describe('DestructuringAssignmentExpression', function () {
            it('{a} = {a: 1, b:2}', function () {
                const bc = {};
                astAssign(new DestructuringAssignmentExpression(27 /* ExpressionKind.ObjectDestructuring */, [
            ...
            it('{prop1, prop2:{prop21}} = {prop1: "foo", prop2: {prop21: 123}}', function () {
                ...
                astAssign(new DestructuringAssignmentExpression(27 /* ExpressionKind.ObjectDestructuring */, [
                    ...
                    new DestructuringAssignmentExpression(27 /* ExpressionKind.ObjectDestructuring */, [
            ...
            it('{prop1,coll:[,{p2:item2p2}]} = {prop1:"foo",coll:[{p1:1,p2:2},{p1:3,p2:4}]}', function () {
                ...
                astAssign(new DestructuringAssignmentExpression(27 /* ExpressionKind.ObjectDestructuring */, [
                    new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'prop1'), new AccessMemberExpression($this, 'prop1'), void 0),
                    new DestructuringAssignmentExpression(26 /* ExpressionKind.ArrayDestructuring */, [
                        new DestructuringAssignmentExpression(27 /* ExpressionKind.ObjectDestructuring */, [
            ...
            it('[k, {prop1, prop2:{prop21}}] = ["key",{prop1: "foo", prop2: {prop21: 123}}]', function () {
                const bc = {};
                astAssign(new DestructuringAssignmentExpression(26 /* ExpressionKind.ArrayDestructuring */, [
            ...
            it('{a,b:{c}={c:42}} = {a:42}', function () {
                const bc = {};
                const expr = new DestructuringAssignmentExpression(27 /* ExpressionKind.ObjectDestructuring */, [
                    new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, 'a'), void 0),
                    new DestructuringAssignmentExpression(27 /* ExpressionKind.ObjectDestructuring */, [
    ...
    describe('arrow function unparsing', function () {
        ...
        it('unparses arrow fn with single rest parameter', function () {
            assert.strictEqual(Unparser.unparse(new ArrowFunction([new BindingIdentifier('a')], new AccessScopeExpression('a'), true)), '(...a) => a');
        });
        it('unparses arrow fn with 2 params', function () {
            assert.strictEqual(Unparser.unparse(new ArrowFunction([new BindingIdentifier('a'), new BindingIdentifier('b')], new AccessScopeExpression('a'))), '(a, b) => a');
        });
        it('unparses arrow fn with 2 params with rest', function () {
            assert.strictEqual(Unparser.unparse(new ArrowFunction([new BindingIdentifier('a'), new BindingIdentifier('b')], new AccessScopeExpression('a'), true)), '(a, ...b) => a');
        });
    });
});
