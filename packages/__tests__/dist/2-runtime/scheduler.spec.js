...
describe('2-runtime/scheduler.spec.ts', function () {
    ...
    it('2 persistent delayed tasks with different delays retain correct timing', async function () {
        let counter10 = 0;
        let counter20 = 0;
        const task10 = platform.taskQueue.queueTask(function () {
            ++counter10;
        }, {
            persistent: true,
            delay: 10,
        });
        const task20 = platform.taskQueue.queueTask(function () {
            ++counter20;
        }, {
            persistent: true,
            delay: 20,
        });
        const task = platform.taskQueue.queueTask(noop, {
            persistent: true,
            delay: 100,
        });
        await task.result;
    ...
    it('3 persistent delayed tasks with different delays retain correct timing', async function () {
        ...
        const task40 = platform.taskQueue.queueTask(function () {
            ++counter40;
        }, {
            persistent: true,
            delay: 40,
