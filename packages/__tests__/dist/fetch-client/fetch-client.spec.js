...
describe('fetch-client/fetch-client.spec.ts', function () {
    ...
    describe('configure', function () {
        ...
        it('accepts configuration callbacks', function () {
            ...
            assert.strictEqual(client.isConfigured, true, `client.isConfigured`);
            assert.strictEqual(client.defaults.foo, true, `(client.defaults as {foo: boolean}).foo`);
        ...
        it('accepts configuration override', function () {
            ...
            client.configure(config => {
                assert.strictEqual(config.defaults.foo, true, `(config.defaults as {foo: boolean}).foo`);
                return config;
            });
        });
        it('rejects invalid configs', function () {
            assert.throws(() => client.configure(1), /invalid config/, `() => client.configure(1 as RequestInit)`);
        });
        it('rejects invalid config returned from "configure" call', function () {
            assert.throws(() => client.configure(() => 1), /The config callback did not return a valid HttpClientConfiguration/, `() => client.configure(1 as RequestInit)`);
        });
    ...
    describe('fetch', function () {
        ...
        beforeEach(function () {
            ...
            window.fetch = function (...args) {
                callCount++;
                return mockFetchFn(...args);
            };
            mockFetchFn = function (request) {
                return {
                    // request,
        ...
        it('errors on missing fetch implementation', function () {
            ...
            assert.throws(() => createFixture('', class App {
                constructor() {
                    this.http = resolve(IHttpClient);
                }
            }));
        ...
        it('makes requests with Request as input', async function () {
            assert.deepStrictEqual(await client.fetch(new Request('/a')), { method: 'GET', url: '/a', headers: {} });
        });
        it('makes proper requests with json() inputs', async function () {
            assert.deepStrictEqual(await client.fetch('http://example.com/some/cool/path', {
                method: 'post',
        ...
        it('uses default headers', async function () {
            client.configure(c => c.withDefaults({ headers: { 'x-test': 'abc', 'x-test2': (() => 'a1') } }));
            assert.deepStrictEqual(await client.fetch('/a'), { method: 'GET', url: '/a', headers: { 'x-test': 'abc', 'x-test2': 'a1' } });
        });
        it('does not override headers set in fetch call', async function () {
            client.configure(c => c.withDefaults({ headers: { 'x-test': 'abc' } }));
            assert.deepStrictEqual(await client.fetch('/a', { headers: { 'x-test': 'def' } }), { method: 'GET', url: '/a', headers: { 'x-test': 'def' } });
        });
        ...
        it('makes request and aborts with an AbortController signal', async function () {
            ...
            const promise = client
                ...
                .then(() => { throw new Error('Invalid path'); }, (error) => {
        ...
        describe('shortcut methods', function () {
            it('sends get', async function () {
                assert.deepStrictEqual(await client.get('/a'), { method: 'GET', url: '/a', headers: {} });
            });
            it('sends post', async function () {
                assert.deepStrictEqual(await client.post('/a'), { method: 'POST', url: '/a', headers: {} });
            });
            it('sends put', async function () {
                assert.deepStrictEqual(await client.put('/a'), { method: 'PUT', url: '/a', headers: {} });
            });
            it('send patch', async function () {
                assert.deepStrictEqual(await client.patch('/a'), { method: 'PATCH', url: '/a', headers: {} });
            });
            it('sends delete', async function () {
                assert.deepStrictEqual(await client.delete('/a'), { method: 'DELETE', url: '/a', headers: {} });
            });
        });
        describe('with baseUrl configured', function () {
            it('compute final url with string inputs', async function () {
                ...
                assert.deepStrictEqual(await client.fetch('some/cool/path'), { method: 'GET', url: 'http://example.com/some/cool/path', headers: {} });
            });
            // fetch client shouldn't try to reconfigure url of a request object
            it('disegards baseUrl of Request inputs', async function () {
                client.configure(config => config.withBaseUrl('http://example.com/'));
                assert.deepStrictEqual(await client.fetch(new Request('some/cool/path')), { method: 'GET', url: '/some/cool/path', headers: {} });
            });
        });
...
//# sourceMappingURL=fetch-client.spec.js.map