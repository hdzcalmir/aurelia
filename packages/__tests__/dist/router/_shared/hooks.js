...
export const routingHooks = ['canUnload', 'canLoad', 'unloading', 'loading'];
...
export function* getStopHooks(root, p, c = '', c3 = '', c4 = '') {
    ...
    if (p) {
        yield* prepend('stop', p, 'unloading', 'detaching');
    }
    yield `stop.${root}.unbinding`;
    if (c) {
        yield* prepend('stop', c, 'unloading', 'detaching');
    }
    if (c3) {
        yield* prepend('stop', c3, 'unloading', 'detaching');
    }
    if (c4) {
        yield* prepend('stop', c4, 'unloading', 'detaching');
    }
    if (p) {
...
export function* getSingleHooks(deferUntil, swapStrategy, componentKind, phase, from, to) {
    ...
    if (from) {
        yield `${phase}.${from}.unloading`;
    }
    if (to) {
        yield `${phase}.${to}.loading`;
    }
    switch (swapStrategy) {
        case 'parallel-remove-first':
...
export function* getParentChildHooks(deferUntil, swapStrategy, componentKind, phase, from, to) {
    ...
    const childAdd = ['canLoad', 'loading', ...addHooks];
    const parentRemove = ['unloading', ...removeHooks];
    ...
    if (from.c) {
        yield `${phase}.${from.c}.unloading`;
    }
    if (from.p) {
        yield `${phase}.${from.p}.unloading`;
    }
    parentRemove.shift();
    yield `${phase}.${to.p}.loading`;
    if (deferUntil === 'load-hooks') {
        if (to.c) {
            yield `${phase}.${to.c}.loading`;
        }
        childAdd.shift();
    }
    switch (deferUntil) {
