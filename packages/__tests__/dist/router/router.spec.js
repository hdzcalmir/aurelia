...
describe('router/router.spec.ts', function () {
    ...
    describe('can use configuration', function () {
        ...
        const appRoutes = [
            ...
            { path: 'parent-config/:id', instructions: [{ component: 'parent', viewport: 'default', children: [{ component: 'child', viewport: 'parent' }] }], title: (instruction) => `ParentConfig${instruction.parameters.get(instruction.scope.router, 'id') ?? ':id'}Config` },
    ...
    describe('can use lifecycleHooks', function () {
        ...
        let Hooks = class Hooks {
            ...
            loading(vm) { calledHooks.push(`${this.name}:${vm.name}:load`); }
            canUnload(vm) { calledHooks.push(`${this.name}:${vm.name}:canUnload`); return true; }
            unloading(vm) { calledHooks.push(`${this.name}:${vm.name}:unload`); }
        };
        Hooks = __decorate([
            lifecycleHooks()
        ], Hooks);
    ...
    describe('can redirect', function () {
        ...
        const Two = CustomElement.define({ name: 'two', template: '!two!', }, class {
            canLoad() { return 'route-zero'; }
        });
        const Three = CustomElement.define({ name: 'three', template: '!three!', }, class {
            canLoad() { return 'zero'; }
        });
        const Four = CustomElement.define({ name: 'four', template: '!four!', }, class {
            canLoad() { return 'zero-id'; }
        });
        const tests = [
    ...
    `,
        ...
        const Two = CustomElement.define({ name: 'two', template: '!two${id}!' }, (_a = class {
                loading(params) { if (params.id != null) {
                    this.id = `:${params.id}`;
                } }
            },
            _a.parameters = ['id'],
        ...
        const TwoRoute = CustomElement.define({ name: 'two-route', template: '!two-route${id}!' }, (_b = class {
                loading(params) { if (params.id != null) {
                    this.id = `:${params.id}`;
                } }
            },
