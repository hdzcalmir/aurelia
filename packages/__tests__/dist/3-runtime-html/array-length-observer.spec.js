...
describe('3-runtime-html/array-length-observer.spec.ts', function () {
    ...
    it('works when bound with input number', function () {
        const { getBy, component, type, flush } = createFixture(`<input value.bind="items.length | number" input.trigger="logChange($event)" />`, class TestClass {
            constructor() {
    ...
    it('notifies subscribers of the owning array', function () {
        const { assertText, type } = createFixture('<input value.bind="items.length | number"><div repeat.for="i of items">${i}', class {
            constructor() {
                this.items = [1, 2, 3];
            }
        }, [NumberVc]);
