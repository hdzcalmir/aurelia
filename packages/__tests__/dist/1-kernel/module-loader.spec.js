...
import { DI, IModuleLoader, aliasedResourcesRegistry } from '@aurelia/kernel';
...
describe('1-kernel/module-loader.spec.ts', function () {
    ...
    describe('aliasedResourcesRegistry', function () {
        let loader;
        const assertElementRegistration = (name, registered) => assert.strictEqual(loader.container.has(CustomElement.keyFrom(name), false), registered);
        const assertAttributeRegistration = (name, registered) => assert.strictEqual(loader.container.has(CustomAttribute.keyFrom(name), false), registered);
        const assertBindingBehaviorRegistration = (name, registered) => assert.strictEqual(loader.container.has(BindingBehavior.keyFrom(name), false), registered);
        const assertValueConverterRegistration = (name, registered) => assert.strictEqual(loader.container.has(ValueConverter.keyFrom(name), false), registered);
        const assertBindingCommandRegistration = (name, registered) => assert.strictEqual(loader.container.has(BindingCommand.keyFrom(name), false), registered);
        ...
        it('registers non resource value as is', function () {
            ...
            let B = (() => {
                let _classDecorators = [customElement({ name: 'b' })];
