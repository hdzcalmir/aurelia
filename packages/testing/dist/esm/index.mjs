...
import { StandardConfiguration as y, IPlatform as x, ITemplateCompiler as $, CustomElement as w, CustomAttribute as k, Aurelia as E, valueConverter as S, bindable as C, customElement as O } from "@aurelia/runtime-html";

import { BrowserPlatform as q } from "@aurelia/platform-browser";

import { Metadata as j } from "@aurelia/metadata";

const {getPrototypeOf: A, getOwnPropertyDescriptor: T, getOwnPropertyDescriptors: F, getOwnPropertyNames: I, getOwnPropertySymbols: M, defineProperty: N, defineProperties: R} = Object;
...
const Q = A(Uint8Array.prototype);

const J = uncurryThis(T(Q, Symbol.toStringTag).get);

const K = uncurryThis(Object.prototype.toString);
...
function isEqualBoxedPrimitive(e, t) {
    if (isNumberObject(e)) {
        return isNumberObject(t) && B(re(e), re(t));
    }
    if (isStringObject(e)) {
...
class TestContext {
    ...
    get platform() {
        if (this.p === void 0) {
            this.p = this.container.get(x);
        }
        return this.p;
    }
    get templateCompiler() {
        if (this.t === void 0) {
            this.t = this.container.get($);
        }
        return this.t;
    }
    get observerLocator() {
...
const Be = L([ [ isUint8Array, Uint8Array ], [ isUint8ClampedArray, Uint8ClampedArray ], [ isUint16Array, Uint16Array ], [ isUint32Array, Uint32Array ], [ isInt8Array, Int8Array ], [ isInt16Array, Int16Array ], [ isInt32Array, Int32Array ], [ isFloat32Array, Float32Array ], [ isFloat64Array, Float64Array ] ]);
...
function clazzWithNullPrototype(e, t) {
    ...
    N(NullPrototype.prototype.constructor, "name", {
...
function formatIterator(e, t, n, i) {
    ...
    if (t instanceof Map) {
        ...
        return formatMapIterInner(e, n, r, Ne);
    }
...
function createFixture(e, t, n = [], i = true, r = TestContext.create()) {
    ...
    if (d !== t && t != null) {
        p.forEach((e => {
            j.define(e, w.getAnnotation(t, e), d);
        }));
    }
...
let Ke = class SortValueConverter {
    toView(e, t, n = "asc") {
        if (Array.isArray(e)) {
            const i = n === "asc" ? 1 : -1;
            if (t?.length) {
...
Ke = __decorate([ S("sort") ], Ke);

let Ge = class JsonValueConverter {
    toView(e) {
...
Ge = __decorate([ S("json") ], Ge);

let Ye = class NameTag {};

__decorate([ C() ], Ye.prototype, "name", void 0);
...
function htmlStringify(e, t) {
    ...
    if (e.textContent != null && e.textContent.length || e.nodeType === 3 || e.nodeType === 8) {
        const t = e.textContent.replace(et, "");
        if (t.length > 10) {
            const e = t.length;
    ...
    if (e.nodeType === 1) {
        if (e.innerHTML.length) {
            const t = e.innerHTML.replace(et, "");
            if (t.length > 10) {
                const e = t.length;
                return `${t.slice(0, 10)}...(+${e - 10})`;
            }
...
function createObserverLocator(e) {
    ...
    a.instance(g, null).register(t);
    a.instance(b, n).register(t);
    return t.get(c);
}

function createScopeForTest(e = {}, t, n) {
    return t ? v.fromParent(v.create(t), e) : v.create(e, null, n);
}
...
export { He as CSS_PROPERTIES, Call, CallCollection, ChangeSet, CollectionChangeSet, Ge as JsonValueConverter, MockBinding, MockBindingBehavior, MockBrowserHistoryLocation, MockContext, MockPropertySubscriber, MockServiceLocator, MockSignaler, MockTracingExpression, MockValueConverter, ge as PLATFORM, be as PLATFORMRegistration, We as PSEUDO_ELEMENTS, ProxyChangeSet, Ke as SortValueConverter, SpySubscriber, Ze as TestConfiguration, TestContext, _, Ue as assert, createContainer, createFixture, createObserverLocator, createScopeForTest, createSpy, eachCartesianJoin, eachCartesianJoinAsync, eachCartesianJoinFactory, ensureTaskQueuesEmpty, fail, generateCartesianProduct, getVisibleText, Qe as globalAttributeNames, h, hJsx, htmlStringify, inspect, instructionTypeName, jsonStringify, onFixtureCreated, padLeft, padRight, recordCalls, setPlatform, stopRecordingCalls, stringify, trace, pe as trimFull, verifyBindingInstructionsEqual, verifyEqual };
