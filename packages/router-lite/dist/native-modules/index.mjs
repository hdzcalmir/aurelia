...
import { DI as i, IEventAggregator as s, ILogger as n, Protocol as r, emptyArray as o, onResolve as h, emptyObject as a, resolveAll as c, IContainer as u, isArrayIndex as l, IModuleLoader as f, InstanceProvider as d, noop as p, Registration as g } from "../kernel/dist/native-modules/index.mjs";

import { isCustomElementViewModel as w, IHistory as v, ILocation as m, IWindow as $, CustomElement as x, Controller as E, IPlatform as R, CustomElementDefinition as b, IController as y, IAppRoot as k, isCustomElementController as S, customElement as C, bindable as I, customAttribute as N, IEventTarget as A, INode as T, getRef as V, CustomAttribute as P, AppTask as U } from "../runtime-html/dist/native-modules/index.mjs";

import { RecognizedRoute as L, Endpoint as O, ConfigurableRoute as j, RESIDUE as D, RouteRecognizer as B } from "../route-recognizer/dist/native-modules/index.mjs";
...
function X(t, e) {
    switch (typeof t) {
      ...
      case "object":
        ...
        if (!w(t) && !G(t)) Z(`an object with at least a 'component' property (see Routeable)`, e, t);
        break;
    ...
    static resolve(t, e, i, s) {
        ...
        return h(o, (s => {
            ...
            if (null === o) {
                const r = s.Type;
                let c = null;
                if (h) c = RouteConfig.u(t.getRouteConfig(e, i) ?? a, r, n); else c = _(t) ? gt.isConfigured(r) ? gt.getConfig(r).applyChildRouteConfig(t, n) : RouteConfig.u(t, r, n) : gt.getConfig(s.Type);
...
class ViewportAgent {
    ...
    cancelUpdate() {
        ...
        switch (this.nextState) {
          ...
          case 4:
          case 1:
            this.$ = h(this.nextCA?.deactivate(null, this.hostController, 0), (() => {
                this.nextCA?.dispose();
                this.$plan = "replace";
                this.nextState = 64;
                this.nextCA = null;
...
class RouteNode {
    ...
    static create(t) {
        ...
        return new RouteNode(++kt, t.path, t.finalPath, t.context, t.originalInstruction ?? t.instruction, t.instruction, i, t.queryParams ?? At, t.fragment ?? null, t.data ?? {}, t.viewport ?? null, t.title ?? null, t.component, t.children ?? [], t.residue ?? []);
    }
    contains(t) {
...
function Ct(t, e, i, s, n, r = s.route.endpoint.route) {
    const o = e.context;
    const a = e.tree;
    return h(r.handler, (h => {
        r.handler = h;
        t.trace(`creatingConfiguredNode(rd:%s, vi:%s)`, h, i);
        if (null === h.redirectTo) {
            const c = (i.viewport?.length ?? 0) > 0 ? i.viewport : h.viewport;
            const u = h.component;
            const l = o.resolveViewportAgent(new ViewportRequest(c, u.name));
            const f = o.container.get(Pt);
            const d = f.getRouteContext(l, u, null, l.hostController.container, o.definition);
        ...
        const u = RouteExpression.parse(h.redirectTo, false);
        let l;
        let f;
        const d = [];
        switch (c.root.kind) {
        ...
        while (!(w && v)) {
            ...
            if (null !== g) if (g.component.isDynamic && (p?.component.isDynamic ?? false)) d.push(s.route.params[g.component.parameterName]); else d.push(g.raw);
        }
        const m = d.filter(Boolean).join("/");
        const $ = o.recognize(m);
        if (null === $) throw new UnknownRouteError(`'${m}' did not match any configured route or registered component name at '${o.friendlyPath}' - did you forget to add '${m}' to the routes list of the route decorator of '${o.component.name}'?`);
...
class Transition {
    ...
    handleError(t) {
        this.R = t instanceof UnknownRouteError;
...
let Ut = class Router {
    get ctx() {
        ...
        if (null === t) {
            ...
            t = this.C = this.container.get(Wt);
        }
        return t;
    }
    get routeTree() {
        ...
        if (null === t) {
            const e = this.ctx;
            t = this.I = new RouteTree(NavigationOptions.create(this.options, {}), At, null, RouteNode.create({
                path: "",
                finalPath: "",
                context: e,
                instruction: null,
    ...
    get currentTr() {
        let t = this.N;
        if (null === t) t = this.N = Transition.create({
            id: 0,
    ...
    getRouteContext(t, e, i, s, r) {
        ...
        const u = s.has(Wt, true) ? s.get(Wt) : null;
    ...
    run(t) {
        ...
        const o = !this.navigated || this.V || t.trigger === (r ? "hashchange" : "popstate") || i.length !== s.length || i.some(((t, e) => !(s[e]?.originalInstruction.equals(t) ?? false))) || "replace" === this.ctx.definition.config.getTransitionPlan(t.previousRouteTree.root, t.routeTree.root);
        if (!o) {
        ...
        t.run((() => {
            ...
            Batch.start((i => {
                ...
                switch (t.options.i(this.instructions)) {
                  ...
                  case "push":
                    this.locationMgr.pushState(Vt(t.options.state, t.id), this.updateTitle(t), e);
                    break;

                  case "replace":
                    this.locationMgr.replaceState(Vt(t.options.state, t.id), this.updateTitle(t), e);
                    break;
                }
...
Ut = st([ nt(0, u), nt(1, R), nt(2, n), nt(3, ot), nt(4, ct), nt(5, it) ], Ut);
...
class ComponentAgent {
    constructor(t, e, i, s, r, o) {
        ...
        this.P = r.container.get(n).scopeTo(`ComponentAgent<${r.friendlyPath}>`);
...
class RouteContext {
    ...
    processDefinition(t) {
        ...
        this.G = true;
        if (e.length > 0) this.B = Promise.all(e).then((() => {
            this.B = null;
        }));
    ...
    createComponentAgent(t, e) {
        ...
        if (!this.G) {
            const t = RouteDefinition.resolve(s, n, e);
...
class NavigationRoute {
    ...
    setIsActive(t, e) {
        this.Y = this.path.some((i => t.isActive(i, e)));
    }
}
...
Xt = st([ N("load"), nt(0, A), nt(1, T), nt(2, Pt), nt(3, ot), nt(4, Wt), nt(5, ct) ], Xt);
...
let te = class HrefCustomAttribute {
    ...
    binding() {
        if (!this.isInitialized) {
            ...
            this.isEnabled = this.isEnabled && null === V(this.el, P.getDefinition(Xt).key);
        }
...
}), nt(0, A), nt(1, T), nt(2, Pt), nt(3, Wt), nt(4, $) ], te);
