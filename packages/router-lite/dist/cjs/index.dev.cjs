...
const IRouterEvents = /*@__PURE__*/ kernel.DI.createInterface('IRouterEvents', x => x.singleton(RouterEvents));
class RouterEvents {
    constructor() {
        /** @internal */ this._subscriptionSerial = 0;
        /** @internal */ this._subscriptions = [];
        /** @internal */ this._ea = kernel.resolve(kernel.IEventAggregator);
        /** @internal */ this._logger = kernel.resolve(kernel.ILogger).scopeTo('RouterEvents');
    }
    publish(event) {
...
const IBaseHref = /*@__PURE__*/ kernel.DI.createInterface('IBaseHref');
const ILocationManager = /*@__PURE__*/ kernel.DI.createInterface('ILocationManager', x => x.singleton(BrowserLocationManager));
/**
...
class BrowserLocationManager {
    constructor() {
        ...
        /** @internal */ this._logger = kernel.resolve(kernel.ILogger).root.scopeTo('LocationManager');
        /** @internal */ this._events = kernel.resolve(IRouterEvents);
        /** @internal */ this._history = kernel.resolve(runtimeHtml.IHistory);
        /** @internal */ this._location = kernel.resolve(runtimeHtml.ILocation);
        /** @internal */ this._window = kernel.resolve(runtimeHtml.IWindow);
        /** @internal */ this._baseHref = kernel.resolve(IBaseHref);
        /** @internal */ this._event = kernel.resolve(IRouterOptions).useUrlFragmentHash ? 'hashchange' : 'popstate';
...
class RouteConfig {
    ...
    static _create(configOrPath, Type) {
        if (typeof configOrPath === 'string' || configOrPath instanceof Array) {
            ...
            const path = ensureArrayOfStrings(config.path ?? Type?.path ?? kernel.emptyArray);
    ...
    _applyFromConfigurationHook(instance, parent, routeNode) {
        ...
        if (typeof instance.getRouteConfig !== 'function')
            return;
        return kernel.onResolve(instance.getRouteConfig(parent, routeNode), value => {
...
function resolveRouteConfiguration(routeable, isChild, parent, routeNode, context) {
    ...
    return kernel.onResolve(ceDef, $ceDef => {
        ...
        if (!routeConfig._configurationFromHookApplied
            ...
            return kernel.onResolve(routeConfig._applyFromConfigurationHook(routeable, parent, routeNode), () => routeConfig);
        }
        return routeConfig;
    });
}
...
class ViewportAgent {
    ...
    constructor(viewport, hostController, ctx) {
        ...
        this._logger = ctx.container.get(kernel.ILogger).scopeTo(`ViewportAgent<${ctx._friendlyPath}>`);
        ...
        Batch._start(b1 => {
            switch (this._nextState) {
                case 32 /* State.nextIsScheduled */:
                    ...
                    switch (this._$plan) {
                        ...
                        case 'replace':
                            b1._push();
                            void kernel.onResolve(this._nextNode.context._createComponentAgent(this.hostController, this._nextNode), ca => {
            ...
            switch (this._$plan) {
                ...
                case 'invoke-lifecycles': {
                    ...
                    void kernel.onResolve(ctx.allResolved, () => kernel.onResolve(kernel.onResolve(kernel.onResolveAll(...next.residue.splice(0).map(vi => createAndAppendNodes(this._logger, next, vi))), () => kernel.onResolveAll(...ctx.getAvailableViewportAgents().reduce((acc, vpa) => {
            ...
            case 256 /* State.currUnloadDone */:
                ...
                switch (this._$plan) {
                    ...
                    case 'replace': {
                        ...
                        tr._run(() => {
                            return kernel.onResolve(curCa._deactivate(initiator, controller), () => {
        ...
        switch (this._$plan) {
            ...
            case 'replace': {
                ...
                Batch._start(b1 => {
                    tr._run(() => {
                        b1._push();
                        return kernel.onResolve(curCA._deactivate(null, controller), () => curCA._dispose());
                    }, () => {
                        b1._pop();
                    });
                })._continueWith(b1 => {
    ...
    _processDynamicChildren(tr, b) {
        ...
        tr._run(() => {
            b._push();
            const ctx = next.context;
            return kernel.onResolve(ctx.allResolved, () => {
                const existingChildren = next.children.slice();
                return kernel.onResolve(kernel.onResolveAll(...next
                    .residue
                    .splice(0)
                    .map(vi => createAndAppendNodes(this._logger, next, vi))), () => kernel.onResolve(kernel.onResolveAll(...ctx
    ...
    _cancelUpdate() {
        ...
        switch (this._currState) {
            ...
            case 256 /* State.currUnloadDone */:
            case 128 /* State.currDeactivate */:
                currentDeactivationPromise = kernel.onResolve(this._curCA?._deactivate(null, this.hostController), () => {
        ...
        switch (this._nextState) {
            ...
            case 1 /* State.nextActivate */: {
                nextDeactivationPromise = kernel.onResolve(this._nextCA?._deactivate(null, this.hostController), () => {
        ...
        if (currentDeactivationPromise !== null && nextDeactivationPromise !== null) {
            this._cancellationPromise = kernel.onResolve(kernel.onResolveAll(currentDeactivationPromise, nextDeactivationPromise), () => {
                this._currTransition = null;
...
class RouteNode {
    ...
    static create(input) {
        ...
        /*        data */ Object.freeze(input.data ?? kernel.emptyObject), 
...
function createAndAppendNodes(log, node, vi) {
    ...
    switch (vi.component.type) {
                ...
                default: {
                    ...
                    if (rr === null || residue === path) {
                        ...
                        const eagerResult = ctx._generateViewportInstruction({
                            ...
                            params: vi.params ?? kernel.emptyObject,
                            open: vi.open,
                            close: vi.close,
        ...
        case 2 /* NavigationInstructionType.CustomElementDefinition */: {
            ...
            return kernel.onResolve(resolveCustomElementDefinition(vi.component.value, rc)[1], ced => {
    ...
    return kernel.onResolve(route.handler, $handler => {
        ...
        if ($handler.redirectTo === null) {
            ...
            return kernel.onResolve(resolveCustomElementDefinition($handler.component, ctx)[1], ced => {
                ...
                return kernel.onResolve(router.getRouteContext(vpa, ced, null, vpa.hostController.container, ctx.config, ctx, $handler), childCtx => {
...
function createFallbackNode(log, rc, node, vi) {
    ...
    const rr = new $RecognizedRoute(new routeRecognizer.RecognizedRoute(new routeRecognizer.Endpoint(new routeRecognizer.ConfigurableRoute(rc.path[0], rc.caseSensitive, rc), []), kernel.emptyObject), null);
...
class Router {
    ...
    get routeTree() {
        ...
        if (routeTree === null) {
            ...
            routeTree = this._routeTree = new RouteTree(NavigationOptions.create(this.options, {}), emptyQuery, null, RouteNode.create({
                ...
                context: ctx,
                instruction: null,
                component: runtimeHtml.CustomElement.getDefinition(ctx.config.component),
    ...
    constructor() {
        ...
        /** @internal */ this._logger = kernel.resolve(kernel.ILogger).root.scopeTo('Router');
        /** @internal */ this._events = kernel.resolve(IRouterEvents);
        /** @internal */ this._locationMgr = kernel.resolve(ILocationManager);
        this.options = kernel.resolve(IRouterOptions);
        this._vpaLookup = new Map();
        this._instructions = ViewportInstructionTree.create('', this.options);
        this._container.registerResolver(Router, kernel.Registration.instance(Router, this));
    }
    /**
    ...
    _run(tr) {
        ...
        tr._run(() => {
            ...
            const log = /*@__PURE__*/ navigationContext.container.get(kernel.ILogger).scopeTo('RouteTree');
            if (vit.isAbsolute) {
            ...
            return kernel.onResolve(navigationContext.allResolved, () => updateNode(log, vit, navigationContext, rootCtx.node));
        }, () => {
    ...
    _cancelNavigation(tr) {
        ...
        if (guardsResult === false) {
            ...
            void kernel.onResolve(this._enqueue(instructions, 'api', tr.managedState, tr), () => {
...
class ComponentAgent {
    ...
    /** @internal */ _routerOptions) {
        ...
        this._logger = _controller.container.get(kernel.ILogger).scopeTo(`ComponentAgent<${_ctx._friendlyPath}>`);
...
const IRouteContext = /*@__PURE__*/ kernel.DI.createInterface('IRouteContext');
...
class RouteContext {
    ...
    _processConfig(config) {
        ...
        for (; i < len; i++) {
            ...
            for (const path of rdResolution.path ?? kernel.emptyArray) {
    ...
    static setRoot(container) {
        ...
        return kernel.onResolve(router.getRouteContext(null, controller.definition, controller.viewModel, controller.container, null, null, null), routeContext => {
    ...
    recognize(path, searchAncestor = false) {
        ...
        return new $RecognizedRoute(result, Reflect.has(result.params, routeRecognizer.RESIDUE)
            ? (result.params[routeRecognizer.RESIDUE] ?? null)
            : null);
    }
    _addRoute(routeable) {
        trace(this._logger, 3165 /* Events.rcAddRoute */, routeable);
        return kernel.onResolve(resolveRouteConfiguration(routeable, true, this.config, null, this), rdConfig => {
            for (const path of rdConfig.path ?? kernel.emptyArray) {
        ...
        if (trees === null) {
            ...
            trees = this._trees = this.path.map(p => {
                ...
                return new ViewportInstructionTree(NavigationOptions.create(routerOptions, { context }), false, [
                    ViewportInstruction.create({
                        recognizedRoute: new $RecognizedRoute(new routeRecognizer.RecognizedRoute(ep, kernel.emptyObject), null),
                        component: p,
                    })
                ], emptyQuery, null);
            });
...
class ViewportCustomElement {
    constructor() {
        ...
        /** @internal */
        this._ctx = kernel.resolve(IRouteContext);
        /** @internal */
        this._logger = (kernel.resolve(kernel.ILogger).scopeTo(`au-viewport<${this._ctx._friendlyPath}>`));
    }
    /** @internal */
    _getFallback(viewportInstruction, routeNode, context) {
        const fallback = this.fallback;
...
class LoadCustomAttribute {
    constructor() {
        ...
        /** @internal */ this._ctx = kernel.resolve(IRouteContext);
        /** @internal */ this._events = kernel.resolve(IRouterEvents);
        /** @internal */ this._locationMgr = kernel.resolve(ILocationManager);
        this.attribute = 'href';
        this.active = false;
    ...
    valueChanged() {
        ...
        const controller = runtimeHtml.CustomElement.for(this._el, { optional: true });
        if (controller !== null) {
...
class HrefCustomAttribute {
    ...
    constructor() {
        ...
        if (this._router.options.useHref &&
            ...
            this._el.nodeName === 'A') {
            const windowName = kernel.resolve(runtimeHtml.IWindow).name;
...
const IStateManager = /*@__PURE__*/ kernel.DI.createInterface('IStateManager', x => x.singleton(ScrollStateManager));
