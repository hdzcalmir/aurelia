...
class AttributeParser {
    constructor() {
        ...
        this._interpreter = kernel.resolve(ISyntaxInterpreter);
...
const getCommandAnnotation = (Type, prop) => getMetadata(kernel.Protocol.annotation.keyFor(prop), Type);
...
class OneTimeBindingCommand {
    ...
    build(info, exprParser, attrMapper) {
        ...
        value = value === '' ? kernel.camelCase(target) : value;
...
class TemplateCompiler {
    ...
    _compileLocalElement(template, context) {
        ...
        for (const localTemplate of localTemplates) {
            ...
            Reflect.defineProperty(LocalDepType, 'name', { value: kernel.pascalCase(name) });
        ...
        const compilationDeps = (context.root.def.dependencies ?? []).concat(context.root.def.Type == null ? kernel.emptyArray : [context.root.def.Type]);
...
TemplateCompiler.register = kernel.createImplementationRegister(ITemplateCompiler);
...
class CompilationContext {
    constructor(def, container, parent, root, instructions) {
        ...
        if (typeof (this.p = hasParent ? parent.p : container.get(kernel.IPlatform)).document?.nodeType !== 'number') {
...
const templateCompilerHooks = (target, context) => {
    ...
    function decorator(t, context) {
        context.metadata[kernel.registrableMetadataKey] = TemplateCompilerHooks.define(t);
        return t;
    }
};
...
exports.IBindingCommandResolver = IBindingCommandResolver;
exports.IInstruction = IInstruction;
exports.IResourceResolver = IResourceResolver;
...
exports.attributePattern = attributePattern;
exports.bindingCommand = bindingCommand;
exports.generateElementName = generateElementName;
exports.templateCompilerHooks = templateCompilerHooks;
